...
def in_neighborhood(x, y, min_dist, li):
    for nx, ny in li:
...
def generate_poisson(point_gen_cnt, min_radius, max_radius, perlin_noise):
    ...
    while process_li:
        x, y = process_li.pop(random.randint(0, len(process_li) - 1))
        ...
        for i in range(point_gen_cnt):
            ...
            if 0 <= new_x <= len(perlin_noise[0]) and 0 <= new_y <= len(perlin_noise):  # Check to see if its within the borders
                if in_neighborhood(new_x, new_y, min_dist, sample_points) == False:  # Check to see if no neighbors are too close
